                ...
                else if (action == "Commando")
                {
                    ...
                    for (int i = 6; i < inputInfo.Length; i+=2)
                    {
                        ...
                        bool isValidMission = Enum.TryParse(
                            missionStateAsString, out Status status);
...
//ISoldier: abstract Soldier -id, first name, last name
//- + IPrivate : Private - salary
//    - +ILieutenantGeneral : LieutenantGeneral->ICollection < IPrivate >
//   -SpecialisedSoldier : abstract SpecialisedSoldier -> Corps(Airforces or Marines)
//      - +IEngineer : Engineer->IColleation < IRepair >
//         - IRepair : Repair - PartName, HoursWorked
//    - +ICommando : Commando->ICollection < IMission >
//       -IMission : Mission->CodeName, Status(inProgress or Finished)
//    - CompleteMission()
//- + ISpy : Spy - codeNumber